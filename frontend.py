...
def images_zip(images_dir: Path, current_filenames: List[str]) -> Optional[bytes]:
    if not images_dir.exists() or not images_dir.is_dir():
        return None
    files = [p for p in images_dir.rglob("*") if p.is_file() and p.name in current_filenames]
    if not files:
        return None
    buf = BytesIO()
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
...
if out:
    ...
    with tab_images:
        ...
        if not specs:
            ...
            current_filenames = [s.get("filename", "") for s in specs if isinstance(s, dict)]
            ...
            if images_dir.exists():
                ...
                if found_any:
                    z = images_zip(images_dir, current_filenames)
                    if z:
                        st.download_button(
